...
Tangent functions are odd but not monotonic. These include $\tan$ and $\tanpi$ \FS{TODO}. As explained, we need not take specific measures for rounding in this case: once we have determined where the extrema are, the right endpoint is computed normally, and the left is rounded down using the $f(x) = -f(-x)$ property.
